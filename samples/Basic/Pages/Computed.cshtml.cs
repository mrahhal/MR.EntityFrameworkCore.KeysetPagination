...
{
	public class ComputedModel : PageModel
	{
		...
		public async Task OnGet(int? after, int? before, bool first = false, bool last = false)
		{
			...
			{
				b.Ascending(x => x.NullableDateComputed).Ascending(x => x.Id);
			};
			...
			{
				var reference = await _dbContext.Users.FirstOrDefaultAsync(x => x.Id == after.Value);
			...
			{
				var reference = await _dbContext.Users.FirstOrDefaultAsync(x => x.Id == before.Value);
